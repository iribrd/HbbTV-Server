...
if(null_br>0):
  ...
  #print(lines)
   if(lines):
        subprocess = subprocess.Popen(['pgrep', '-f','./DtPlay'], stdout=subprocess.PIPE)
        l, e = subprocess.communicate()
        if(l):
           os.system(final_mux_cmd)
	else: 
          os.system('screen -dms DtPlay python /home/Hbb-server/darabi/server_backend/startup_script.py')
